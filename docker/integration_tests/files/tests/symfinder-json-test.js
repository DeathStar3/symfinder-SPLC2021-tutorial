...
describe("ENTRY_POINT annotations", () => {

    var jsonData, jsonStatsData;

    beforeAll(async (done) => {
        const [graph, stats] = await getJsonData("tests/data/api_annotations.json", "tests/data/api_annotations-stats.json");
        jsonData = graph;
        jsonStatsData = stats;
        done();
    });

    it('Composed1 should be an entry point', () => {
        expect(getNodeWithName(jsonData, "Composed1").types).toContain("ENTRY_POINT");
    });
...
function getNodeWithName(jsonData, name) {
    return jsonData.allnodes.filter(n => n.name === name)[0];
}