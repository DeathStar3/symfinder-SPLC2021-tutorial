...
describe("Tests on attribute_usage", () => {

    var jsonData, jsonStatsData;

    beforeAll(async (done) => {
        const [graph, stats] = await getJsonData("tests/data/attribute_usage.json", "tests/data/attribute_usage-stats.json");
        jsonData = graph;
        jsonStatsData = stats;
        done();
    });

    it('there are 4 usage links', () => {
        expect(jsonStatsData.nbUsageClasses).toBe(4);
    });
