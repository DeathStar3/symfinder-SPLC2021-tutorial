...
def generate_visualization_files_for_project(xp_name, xp_config):
    ...
    if tags_and_commits:
        for id in tags_and_commits:
            ...
            generate_visualization_file_usage(xp_codename, xp_config)
    else:
        generate_visualization_file(xp_name, xp_config)
        generate_visualization_file_usage(xp_name, xp_config)
    generate_index()
...
def generate_visualization_file(xp_codename, xp_config):
    ...
    with open(xp_html_file_path, 'w+') as output_file:
        usageLevel2=1
        usageType2="IN"
        if("usageLevel" in xp_config):
            usageLevel2=int(float(xp_config.get("usageLevel")))
        if("usageType" in xp_config):
            usageType2=str(xp_config.get("usageType"))
        ...
        output_file.write(Template(filename=os.path.join(d3_directory, "template.html")).render(
            ...
            entryPointFilters=",".join(['"' + f + '"' for f in xp_config.get("entryPointFilters", [])]),
            usageLevel=usageLevel2,
...
def generate_visualization_file_usage(xp_codename, xp_config):
    xp_html_file_path = os.path.join(base_directory, "%s-usage.html" % xp_codename)
    with open(xp_html_file_path, 'w+') as output_file:
        usageLevel1=1
        usageType1= "IN"
        if("usageLevel" in xp_config):
            usageLevel1=int(float(xp_config.get("usageLevel")))
        if("usageType" in xp_config):
            usageType1=str(xp_config.get("usageType"))

        output_file.write(Template(filename=os.path.join(d3_directory, "templateusage.html")).render(
            title=xp_codename,
            identifier="{} generated by symfinder version {}".format(xp_codename,
                                                                     os.getenv("SYMFINDER_VERSION")),
            jsScriptFile=os.path.join("symfinder.js"),
...
def generate_index():
    with open(os.path.join(base_directory, "index.html"), 'w+') as index_file:
        ...
        usage_visualizations = {}
        for f in os.listdir(base_directory):
            if f.endswith(".html") and f != "index.html":
                if f.endswith("-usage.html"):
                    usage_visualizations[f.split("-usage.html")[0]] = f
                else:
                    standard_visualizations[f.split(".html")[0]] = f
        index_file.write(Template(filename=os.path.join(d3_directory, "index.html")).render(
            xps=sort_dictionary(standard_visualizations),
            usage_xps=sort_dictionary(usage_visualizations)
        ))
